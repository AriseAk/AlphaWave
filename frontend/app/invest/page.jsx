...
import { TrendingUp, TrendingDown, DollarSign, ArrowUpRight, ArrowDownRight, Plus, Search, Sparkles , User , LogOut } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 relative z-10">
        ...
        <div className="mb-4 animate-fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-white bg-opacity-90 backdrop-blur-xl rounded-xl p-4 border border-blue-100 shadow-lg col-span-2 transform hover:scale-105 transition-all duration-500 hover:shadow-2xl animate-fade-in" style={{ animationDelay: '100ms' }}>
              <div className="flex items-center justify-between mb-2">
                <div>
                  <p className="text-xs text-gray-600 mb-1">Total Portfolio Value</p>
                  <h2 className="text-2xl font-bold text-gray-900 transition-all duration-300">
                    ${portfolioValue.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                  </h2>
                </div>
                <div className="w-12 h-12 bg-linear-to-br from-blue-500 to-purple-500 rounded-full flex items-center justify-center animate-pulse-slow shadow-lg">
              ...
              <div className="flex items-center space-x-3">
                <div className={`flex items-center ${totalGainPercent >= 0 ? 'text-green-600' : 'text-red-600'} transition-all duration-500`}>
                  {totalGainPercent >= 0 ? <ArrowUpRight className="w-4 h-4 animate-bounce" /> : <ArrowDownRight className="w-4 h-4 animate-bounce" />}
            ...
            <div className="bg-linear-to-br from-blue-500 via-purple-500 to-pink-500 rounded-xl p-4 text-white shadow-2xl transform hover:scale-105 transition-all duration-500 hover:rotate-1 relative overflow-hidden animate-fade-in" style={{ animationDelay: '200ms' }}>
              <div className="absolute inset-0 bg-white opacity-0 hover:opacity-10 transition-opacity duration-300" />
              <p className="text-xs text-blue-100 mb-1">Buying Power</p>
              <h3 className="text-2xl font-bold mb-2">$74,659.50</h3>
              <button className="w-full bg-white text-blue-500 py-2 rounded-lg hover:bg-blue-50 transition-all duration-300 font-medium flex items-center justify-center transform hover:scale-105 hover:shadow-lg text-sm">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="bg-white bg-opacity-90 backdrop-blur-xl rounded-xl border border-blue-100 shadow-lg animate-fade-in" style={{ animationDelay: '300ms' }}>
              <div className="p-4 border-b border-blue-100">
                <div className="flex items-center justify-between">
                  <h2 className="text-lg font-bold text-gray-900">Holdings</h2>
                  <div className="relative group">
                    <Search className="absolute left-2 top-1/2 transform -translate-y-1/2 w-3 h-3 text-gray-400 transition-all duration-300 group-hover:scale-110" />
                    <input
                      type="text"
                      value={searchStock}
                      onChange={(e) => setSearchStock(e.target.value)}
                      className="pl-8 pr-3 py-1.5 border-2 border-blue-200 rounded-lg focus:ring-2 focus:ring-blue-300 focus:border-transparent text-xs transition-all duration-300"
              ...
              <div className="divide-y divide-blue-100">
                {holdings.map((holding, index) => (
                  <div 
                    key={index} 
                    className="p-3 hover:bg-linear-to-r hover:from-blue-50 hover:to-purple-50 transition-all duration-500 cursor-pointer"
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <div className="flex items-center space-x-2">
                          <div className={`w-10 h-10 bg-linear-to-br ${holding.color} rounded-lg flex items-center justify-center transform transition-all duration-500 ${hoveredHolding === index ? 'rotate-12 scale-110' : ''} shadow-lg`}>
                            <span className="font-bold text-white text-xs">{holding.symbol}</span>
                          </div>
                          <div>
                            <h3 className="font-semibold text-gray-900 text-sm">{holding.symbol}</h3>
                      ...
                      <div className="text-right mr-6">
                        <p className="font-semibold text-gray-900 text-sm">{holding.shares} shares</p>
                        <p className="text-xs text-gray-500">${holding.price}</p>
                      </div>

                      <div className="text-right">
                        <p className="font-semibold text-gray-900 text-sm">${holding.value.toLocaleString()}</p>
                        <div className={`flex items-center justify-end text-xs font-medium ${holding.change >= 0 ? 'text-green-600' : 'text-red-600'} transition-all duration-300`}>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white bg-opacity-90 backdrop-blur-xl rounded-xl border border-blue-100 shadow-lg animate-fade-in" style={{ animationDelay: '500ms' }}>
              ...
              <div className="divide-y divide-blue-100">
                {watchlist.map((stock, index) => (
                  <div 
                    key={index} 
                    className="p-3 hover:bg-linear-to-r hover:from-blue-50 hover:to-purple-50 transition-all duration-300 cursor-pointer transform hover:scale-105"
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className={`flex items-center text-xs font-medium transition-all duration-300 ${stock.change >= 0 ? 'text-green-600' : 'text-red-600'} ${hoveredWatchlist === index ? 'scale-110' : ''}`}>
              ...
              <div className="p-3 border-t border-blue-100">
                <button className="w-full text-blue-500 hover:text-blue-600 font-medium text-xs transition-all duration-300 hover:scale-105">
                  + Add to Watchlist
                </button>
              </div>
            </div>
            ...
            {/* Quick Stats */}
            <div className="bg-linear-to-br from-blue-100 via-purple-100 to-pink-100 rounded-xl p-4 border border-blue-200 shadow-lg transform hover:scale-105 transition-all duration-500 animate-fade-in" style={{ animationDelay: '600ms' }}>
              <h3 className="font-semibold text-gray-900 mb-3 text-sm">Performance</h3>
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-xs text-gray-600">Today&apos;s Return</span>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs font-semibold text-gray-900">$100,000.00</span>
                </div>
                <div className="flex justify-between items-center">
            ...
            {/* Action Button */}
            <button className="w-full bg-linear-to-r from-blue-500 via-purple-500 to-pink-500 text-white py-3 rounded-xl hover:shadow-2xl transition-all duration-500 font-medium flex items-center justify-center transform hover:scale-105 hover:-translate-y-1 relative overflow-hidden group animate-fade-in text-sm" style={{ animationDelay: '700ms' }}>
